    ...
    buildPublicMetadata() {
        ...
        if (this.serverConfig.dataSets) {
            for (let dsCode in this.serverConfig.dataSets) {
                ...
                } else if (dsMetadata.type == "tiles") {
                    dsMetadata.temporality = typeof dsConfig.temporality == "object"?{value:dsConfig.temporality.value, unit:dsConfig.temporality.unit}:dsConfig.temporality;
                    dsMetadata.maps = [];
                    for (let mapName in dsConfig.maps) {
                        let map = dsConfig.maps[mapName];
                        let mapMetadata = JSON.parse(JSON.stringify(map.metadata || {}));
                        mapMetadata.name = mapName;
                        mapMetadata.commonName = map.commonName || mapName;
                        mapMetadata.urlPattern = map.urlPattern;
                        let mapOptions =  map.options || {};
                        mapOptions = JSON.parse(JSON.stringify(mapOptions));
                        if (dsConfig.mapsDefault && dsConfig.mapsDefault.options) {
                            this.extendOptions(mapOptions, dsConfig.mapsDefault.options);
                        }
                        mapMetadata.options = mapOptions;
                        dsMetadata.maps.push(mapMetadata);
                    }
                }
