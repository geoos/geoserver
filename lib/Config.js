    ...
    buildPublicMetadata() {
        ...
        if (this.serverConfig.dataSets) {
            for (let dsCode in this.serverConfig.dataSets) {
                ...
                if (dsConfig.otions) dsMetadata.options = dsConfig.options;
                if (dsMetadata.type == "raster") {
                    dsMetadata.temporality = {value:dsConfig.temporality.value, unit:dsConfig.temporality.unit}
                    ...
                    for (let varCode in dsConfig.variables) {
                        ...
                        varMetadata.queries = [];
                        if (dsConfig.commonQueries) dsConfig.commonQueries.forEach(q => varMetadata.queries.push(q))
                        if (variable.queries) variable.queries.forEach(q => varMetadata.queries.push(q))
