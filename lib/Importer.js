...
const fs = require("fs");
const RasterImporter = require("./importers/RasterImporter");
const GeoJsonImporter = require("./importers/GeoJsonImporter");
...
class Importer {
    ...
    async importDaemon() {
        try {
            //log.debug("import daemon");
            if (!config.serverConfig) {
                //log.warn("No server config")
                return;
            }
            ...
            //log.debug("input files in '" + importPath + "':" + JSON.stringify(inputFiles))
            for await (let fileName of inputFiles) {
                ...
                try {
                    ...
                    dataSet = serverConfig.dataSets[dataSetCode];
                ...
                if (dataSet.config.dataSet.type == "raster") {
                    try {
                        await RasterImporter.importFile(dataSetCode, dataSet, fileName, workingFile, config.serverConfig);
                        try {
                ...
                } else if (dataSet.config.dataSet.type == "vector") {
                    try {
                        await GeoJsonImporter.importFile(dataSetCode, dataSet, fileName, workingFile, config.serverConfig);                        
                    } catch(error) {
                        log.error(`Error importing file ${fileName}: ${error.toString()}`);
                        log.error("  -> file moved to /home/data/files-with-errors");
                        try {
