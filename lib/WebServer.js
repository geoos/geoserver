...
class WebServer {
    ...
    registerEndPoints() {
        ...
        for (let dsCode in config.serverConfig.dataSets) {
            ...
            let server = AbstractServer.getServerForDataSource(dataSet);
            if (server) {
                log.debug("starting " + server.constructor.name + " web server for " + dsCode)
                server.registerEndPoints(this.app, dsCode);
            }
        }        
    }
