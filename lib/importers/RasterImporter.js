...
class RasterImporter {
    ...
    async executePreprocessor(preprocessor) {
        try {
            let st = preprocessor;
            let p0 = st.indexOf("${input_filename}");
            if (p0 >= 0) {
                st = st.substring(0, p0) + this.filePath + st.substring(p0 + 17);                
            }
            p0 = this.getFileName().lastIndexOf(".");
            let ext = this.getFileName().substring(p0 + 1);
            let outName = this.getRandomFilePath(ext);
            p0 = st.indexOf("${output_filename}");
            if (p0 >= 0) {
                st = st.substring(0, p0) + outName + st.substring(p0 + 18);
            }
            log.debug("Ejecutando preprocesador: " + st)
            let out = await this.exec(st);
            // Eliminar archivo original y renombrar salida como el original
            fs.unlinkSync(this.filePath);
            fs.renameSync(outName, this.filePath);

        } catch (error) {
            console.error("Error ejecutando preprocesador: " + preprocessor);
            console.error(error);
    ...
    async import() {
        try {
            if (this.dataSet.config.preprocessors && this.dataSet.config.preprocessors.length) {
                for (let pp of this.dataSet.config.preprocessors) {
                    await this.executePreprocessor(pp);
                }
            }
