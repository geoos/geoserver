...
class TimeHelper {
    ...
    getTimeInFileName(dataSetCode, dataSet, fileName, fromPos) {
        ...
        if (time.unit == "days" && time.value == 1) {
            ...
            return fileTime
        } else if (time.unit.toLowerCase().startsWith("minute")) {
            // timeformat: UTC: YYYY-MM-DD_HH-mm
            let dt = fileName.substr(fromPos?fromPos:dataSetCode.length + 1);
            console.log("parseando dt:'" + dt + "'");
            let fileTime = moment.tz(dt, "YYYY-MM-DD_HH-mm", "UTC");
    ...
    validateTimeForDataSet(dataSetCode, dataSet, timeValue) {
        ...
        if (time.unit == "days" && time.value == 1) {
            ...
            // timeformat: UTC: YYYY-MM-DD_HH-mm
            if (timeValue.seconds()) throw "Invalid file time: Seconds should be 00 for 'minutes' time specification";
            let mm = timeValue.minutes();            
            if (mm % time.value) throw "Invalid file time: Minutes should be a multiple of " + time.value; 
            return;
        }
        throw "Unhandled time specification: " + JSON.stringify(time);
    }
    ...
    normalizeTime(dataSet, timeValue) {
        ...
        if (time.unit == "days" && time.value == 1) {
            ...
            let startOfPeriod = timeValue.clone();
            let mm = parseInt(startOfPeriod.minutes() / time.value) * time.value
            startOfPeriod.minutes(mm);
            startOfPeriod = startOfPeriod.startOf("minute");
            let middleOfPeriod = startOfPeriod.clone().add(time.value / 2, "minutes");
            switch(time.searchCriteria) {
                case "start":
                    return {time:startOfPeriod, searchDirection:timeValue.isBefore(middleOfPeriod)?-1:1}
                case "middle":
                    return {time:timeValue.isBefore(middleOfPeriod)?startOfPeriod:startOfPeriod.add(time.value, "minutes"), searchDirection:timeValue.isBefore(middleOfPeriod)?1:-1}
                case "end":
                    return {time:startOfPeriod.add(time.value, "minutes"), searchDirection:timeValue.isBefore(middleOfPeriod)?-1:1}
                d
                efault:
                    throw "Invalid searchCriteria: '" + time.searchCriteria + "'";
            }
    ...
    incTime(dataSet, timeValue, direction) {
        ...
        if (time.unit == "days" && time.value == 1) {
            ...
            return timeValue.add(direction * time.value, "minutes");
        } else throw "Time unit '" + time.unit + "' not handled yet";
    }
    ...
    getTimeInBand(dataSet, band) {
        let att = dataSet.config.temporality.timeMetadataAttribute;
        let value = band.metadata[""][att];
        value = parseInt(value);
        if (isNaN(value)) throw "Invalid time in band";
        let bandTime = moment.tz(1000 * value, "UTC");
        return bandTime;
    }
}
