...
class RasterServer extends AbstractServer {
    ...
    async grid(dataSetCode, dataSet, varCode, params) {
        try {
            ...
            if (dataSet.config.grid && dataSet.config.grid.maxWidth) maxWidth = dataSet.config.grid.maxWidth;
            if (dataSet.config.grid && dataSet.config.grid.maxHeight) maxHeight = dataSet.config.grid.maxHeight;
            if (dataSet.config.grid && dataSet.config.grid.resamplig) resamplig = dataSet.config.grid.resamplig;
    ...
    async vectorsGrid(dataSetCode, dataSet, varCode, params) {
        try {
            ...
            if (!variable.vector || !variable.vector.u || !variable.vector.v) {
            ...
            {
                let {varMetadata, foundTime, dataFilePath} = await this.findVarMetadata(dataSetCode, dataSet, varCodeU, searchTime)
                if (!varMetadata) return {errorCode:"notFound", errorText:"No data"}
